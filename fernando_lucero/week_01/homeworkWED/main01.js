...
var array2 = [];//sets up an array to store letters guessed by user

var setWord = function(word) {
	array1 = word.split('');//takes the word to guess and turns it into an array.
	array2 = [];//sets array2 to empty so as not to add to previous input

	for (var i = 0; i < word.length; i++) {
		array2.push('_');//cycles through length of the word to be guessed and 
						//pushes the amount of empty spaces into array2 to be used in the game
	}
};

var guessLetter = function(letter) {//function to take user input guess letters
    var guessCorrect = false;
    var guessWrong = false;
    // sets up the variables for win/loss conditions

    for (var i = 0; i < array1.length; i++) {
        if (array1[i] === letter) {//cycles through array1 and if letter matches, 
        ...
        if (array2.indexOf('_') === -1){//searches for '_' in array2, if none is found (meaning -1) then all
        								//letters have been replaced with correct letters and user has won
            console.log('You Won!');
        }
        if (guessWrong) {
